...
One fundamental hypothesis of the model is that all observations are independent and identically distributed (*iid*). This is typically not the case for *time series*.
...
For the example, we'll focus on the **Student test**. The test is meant to check whether the true value of the mean is equal to a specific value. Example, I want to test whether the average speed for all activities is 20km/h. Our test is the following :
...
$$T = \sqrt{n} \dfrac{\bar{A}-20}{\sigma} \hookrightarrow \mathcal{N}(0,1)$$
...
$$T = \sqrt{n} \dfrac{\bar{A}-20}{\hat{s}} \hookrightarrow \mathcal{St}_{n-1}$$
...
In our case, this probability is very small, meaning that we should not consider that the average speed of all activities is 20km/h
