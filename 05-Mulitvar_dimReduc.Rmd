...
- Initiate a low-dimensional space and map all high-dimensional points on it. Compute the same pairwise similarities in this space $\Rightarrow$ n collections (distributions) of similarity scores in the low-dimensional space
...
dd %>% mutate(xx_ref=dd$xx[1],yy_ref=dd$yy[1],
  ...
  ggplot(aes(euclid,simil)) + geom_point() + theme_minimal() + labs(title="Distance and similarity to the first point")
